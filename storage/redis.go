...
func (r *RedisClient) GetShareCredits(login string) (float64, int64, error) {
	cmd := r.client.HGetAllMap(r.formatKey("miners", login))
	...
	sharecredits, _ := strconv.ParseFloat(values["sharecredits"], 64)
	openshares, _ := strconv.ParseInt(values["openshares"], 10, 64)
	return sharecredits, openshares, nil
}
...
func (r *RedisClient) WriteShareCredit(login string, amount int64, sharecredit float64, numshares int64) error {
	tx := r.client.Multi()
	defer tx.Close()

	ts := util.MakeTimestamp() / 1000
	remscore := ts - 3600*24*40 // Store the last 40 Days

	addStr := join(amount, numshares)

	_, err := tx.Exec(func() error {
		tx.HIncrByFloat(r.formatKey("miners", login), "sharecredits", -sharecredit)
		tx.HIncrBy(r.formatKey("miners", login), "openshares", -numshares)
		tx.HIncrBy(r.formatKey("miners", login), "balance", amount)
		tx.ZAdd(r.formatKey("rewards", login), redis.Z{Score: float64(ts), Member: addStr})
...
func (r *RedisClient) WriteImmatureBlock(block *BlockData, roundRewards ...map[string]int64) error {
	...
	_, err := tx.Exec(func() error {
		...
		if len(roundRewards) > 0 {
			total := int64(0)
			for login, amount := range roundRewards[0] {
				total += amount
...
func (r *RedisClient) WriteMaturedBlock_pps(block *BlockData) error {
	tx := r.client.Multi()
	defer tx.Close()
...
func (r *RedisClient) WriteMaturedBlock_pplns(block *BlockData, roundRewards map[string]int64) error {
