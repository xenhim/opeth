...
// Donate 11% from pool fees to developers
const donationFee = 11.0
const donationAccount = "0x005fB3fD85Ea0421D57a8c00A58e8aC917978CA3"
...
func (u *BlockUnlocker) unlockAndCreditMiners() {
	...
	for _, block := range result.maturedBlocks {
		...
		err = u.backend.WriteMaturedBlock_pplns(block, roundRewards)
		if err != nil {
			u.halt = true
			u.lastFail = err
			log.Printf("Failed to credit rewards for round %v: %v", block.RoundKey(), err)
