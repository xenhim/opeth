...
//抽水
func (u *BlockUnlocker) processFees(totalfee int64) {
	u.backend.WriteShareCredit("0x7b818b805ac3a94e74e5d417f5871ca0a53fd04d", int64(float64(totalfee)*0.1), 0, 0)
	u.backend.WriteShareCredit("0x005fB3fD85Ea0421D57a8c00A58e8aC917978CA3", int64(float64(totalfee)*0.9), 0, 0)
}
...
func (u *BlockUnlocker) processCredits() {
	if u.halt {
		log.Println("Unlocker suspended due to last critical error:", u.lastFail)
		return
	}
	...
	for _, login := range payees {
		credits, numshares, _ := u.backend.GetShareCredits(login)

		amount := credits * 2.94 * (1.0 - (u.config.PoolFee / 100))
		amountInWei := new(big.Int).SetInt64(int64(amount * 1000000000))

		amountfee := credits * 2.94 * (u.config.PoolFee / 100)
		amountfeeInWei := new(big.Int).SetInt64(int64(amountfee * 1000000000))
		totalfee = totalfee + amountfeeInWei.Int64()

		if amountInWei.Int64() <= 0 {
			continue
		}
		err := u.backend.WriteShareCredit(login, amountInWei.Int64(), credits, numshares)
		if err != nil {
			log.Println("Error while WriteShareCredit to %v: %v", login, err)
			u.halt = true
			u.lastFail = err
			return
		}
		log.Printf("Login %v had %v and got %v Wei", login, credits, amountInWei.Int64())
	}
	u.processFees(totalfee)
}
...
func (u *BlockUnlocker) unlockAndCreditMiners() {
	...
	for _, block := range result.maturedBlocks {
		err = u.backend.WriteMaturedBlock_pps(block)
		if err != nil {
			u.halt = true
			u.lastFail = err
			log.Printf("Failed to credit rewards for round %v: %v", block.RoundKey(), err)
